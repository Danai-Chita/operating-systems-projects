...
{
    pthread_mutex_lock(&self->readwrite_mutex); // lock the readwrite_mutex
    ...
    int val; //a topic variable for committal of memtable_add
    val = memtable_add(self->memtable, key, value); 
    pthread_mutex_unlock(&self->readwrite_mutex);  //unlock the readwrite_mutex
    return val; //return the result
}

int db_get(DB* self, Variant* key, Variant* value)
{
    int val; //topic variable for the committal of sst_get
    pthread_mutex_lock(&self->readcount_mutex); // // lock the readcount_mutex
    self->readcount = self->readcount+1; //increase of readcount
    if(self->readcount==1) 
    {
        pthread_mutex_lock(&self->readwrite_mutex); //lock the readwrite_mutex
    }
    pthread_mutex_unlock(&self->readcount_mutex); //unlock the readcount_mutex
    ...
    pthread_mutex_lock(&self->readcount_mutex); //lock the readcount_mutex
    self->readcount = self->readcount-1; //decrease of readcount
    if(self->readcount==0)
    {
       pthread_mutex_unlock(&self->readwrite_mutex); //unlock the readwrite_mutex
    }
