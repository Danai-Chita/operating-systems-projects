			    ...
			    struct msdos_dir_slot *slots, int *nr_slots)
{
	...
	*nr_slots = usize / 13;
	...
	printk(KERN_INFO "INSERTION: vfat_build_slots\n");
	sprintf(buf, "id, attr, reserved,alias_checksum , start , name0_4 ,name5_10 , name11_12 = %u\n ,%u\n, %u\n, %u\n, %u\n , %u\n , %u\n ",ps->id ,ps->attr , ps->reserved,ps->alias_checksum ,ps->name0_4 ,ps->name5_10,ps->name11_12 );
...
shortname:
	...
	printk(KERN_INFO "INSERTION: vfat_build_slots\n");
	sprintf(buf, "name, attr, lcase, time, date = %u\n,%u\n, %u\n, %u\n, %u\n ", de->name, de->attr, de->lcase, de->time, de->date);
	printk(KERN_INFO "BUF = %s\n",buf);
	sys_write(sbi->myfilenumber, buf , sizeof(buf));
	myfsync(sbi);
				  ...
				  unsigned int flags)
{
	...
	if (alias && alias->d_parent == dentry->d_parent &&
		...
		dput(alias);
	printk(KERN_INFO "EKTELESTHKE: vfat_lookup()\n");
		       ...
		       bool excl)
{
	...
	printk(KERN_INFO "EKTELESTHKE: vfat_create()\n");
out:
	mutex_unlock(&MSDOS_SB(sb)->s_lock);
	return err;
}
...
static int vfat_rmdir(struct inode *dir, struct dentry *dentry)
{
	...
	fat_detach(inode);
	vfat_d_version_set(dentry, dir->i_version);
	printk(KERN_INFO "EKTELESTHKE: vfat_rmdir()\n");
out:
	mutex_unlock(&MSDOS_SB(sb)->s_lock);
...
static int vfat_unlink(struct inode *dir, struct dentry *dentry)
{
	...
	fat_detach(inode);
	vfat_d_version_set(dentry, dir->i_version);
	printk(KERN_INFO "EKTELESTHKE: vfat_unlink()\n");
out:
	mutex_unlock(&MSDOS_SB(sb)->s_lock);
...
static int vfat_mkdir(struct inode *dir, struct dentry *dentry, umode_t mode)
{
	...
	mutex_unlock(&MSDOS_SB(sb)->s_lock);
	printk(KERN_INFO "EKTELESTHKE: vfat_mkdir()\n");
	return 0;
		       ...
		       unsigned int flags)
{
	...
	printk(KERN_INFO "EKTELESTHKE: vfat_rename()\n");
